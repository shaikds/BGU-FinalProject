...
import { getGameById, getGameTitle } from "@/lib/games-data"
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { Input } from "@/components/ui/input"
import { Button } from "@/components/ui/button"
import { ArrowUpDown, Search, TrendingUp, Trophy, Target, ArrowRight } from "lucide-react"
...
// נתונים לדוגמה - ניתן להחליף בנתונים אמיתיים
const generateMockData = (gameId: string) => {
  const players = []
  const seed = parseInt(gameId) || 1
  for (let i = 1; i <= 20; i++) {
    const playerStats: Record<string, number> = { id: i }
    ACTIONS.forEach((action, idx) => {
      playerStats[action] = Math.floor(((seed * i * (idx + 1)) % 15) + Math.random() * 5)
    })
...
export default function GameStatsPage() {
  ...
  const game = useMemo(() => getGameById(parseInt(gameId) || 1), [gameId])
  const gameTitle = getGameTitle(game)
  const [playerData] = useState(() => generateMockData(gameId))
  ...
  const stats = useMemo(() => {
    const totalActions = playerData.reduce((sum, player) => {
      return sum + ACTIONS.reduce((actionSum, action) => actionSum + (player[action] || 0), 0)
    }, 0)
    const totalGoals = playerData.reduce((sum, player) => sum + (player.GOAL || 0), 0)
    const totalPasses = playerData.reduce((sum, player) => sum + (player.PASS || 0), 0)
    return { totalActions, totalGoals, totalPasses }
  }, [playerData])

  // סינון ומיון
  const filteredAndSortedData = useMemo(() => {
    let filtered = playerData.filter((player) => player.id.toString().includes(searchTerm))
  ...
  const chartData = useMemo(() => {
    return playerData
      .map((player) => ({
        id: player.id,
        value: player[selectedAction] || 0,
      }))
      .sort((a, b) => b.value - a.value)
      .slice(0, 15) // מציג רק 15 השחקנים המובילים
  }, [playerData, selectedAction])

  const getBarColor = (index: number, total: number) => {
    if (index === hoveredBar) return "oklch(0.75 0.20 90)" // צהוב בהיר בהובר
    if (index < 3) return "oklch(0.88 0.15 135)" // ירוק בהיר - מובילים
    if (index < 8) return "oklch(0.70 0.18 150)" // ירוק כהה יותר - בינוניים
    return "oklch(0.55 0.12 145)" // ירוק עמוק - אחרים
  }
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-balance">{gameTitle}</h1>
                <p className="text-muted-foreground text-sm">
                  משחק #{gameId} &middot; {game.date} &middot; {game.time} &middot; מעקב ביצועים ופעולות בזמן אמת
                </p>
              </div>
            </div>
            <Button 
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-primary">{stats.totalActions}</p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-secondary">{stats.totalGoals}</p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-primary">{stats.totalPasses}</p>
                  </div>
                  <div className="h-12 w-12 rounded-xl bg-primary/20 flex items-center justify-center">
                    <TrendingUp className="h-6 w-6 text-primary" />
                  </div>
            ...
            <div className="h-[400px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
                  ...
                  <Tooltip
                    contentStyle={{
                      backgroundColor: "oklch(0.20 0.03 145)",
                      border: "2px solid oklch(0.65 0.15 140)",
                      borderRadius: "8px",
                      padding: "12px",
                    }}
        ...
        <Card className="border-primary/30 bg-card/80 backdrop-blur shadow-lg">
          ...
          <CardContent>
                  ...
                  <TableRow className="bg-muted/50 hover:bg-muted/50 border-b-2 border-primary/30">
                    <TableHead className="sticky right-0 bg-muted/50 z-20 min-w-[100px]">
                      <Button
                        ...
                        className="h-8 font-bold hover:bg-primary/20 hover:text-primary"
                      >
                        ID שחקן
                    ...
                    {ACTIONS.map((action) => (
                      <TableHead key={action} className="text-center min-w-[140px]">
                        <Button
                          ...
                          className="h-8 whitespace-nowrap w-full hover:bg-primary/20 hover:text-primary font-medium"
                        >
                          {action}
                    ...
                    >
                      ...
                      {ACTIONS.map((action) => {
                        ...
                        return (
                          ...
                          >
                            <div
                              className={`inline-flex items-center justify-center min-w-[48px] h-10 px-3 rounded-lg transition-all duration-200 ${getColorIntensity(
                                value,
                              )} ${hoveredCell === cellKey ? "scale-110 shadow-lg" : ""}`}
                            >
                              {value}
                            </div>
                          </TableCell>
            ...
            {filteredAndSortedData.length === 0 && (
              <div className="text-center py-12 text-muted-foreground">לא נמצאו שחקנים התואמים את החיפוש</div>
            )}
          </CardContent>
        </Card>
      </div>
